...
window.calculateWalls = function (userVillage, staticData, userTH) {
    ...
    // Calculate user's completed cost and build level distribution
    let userWalls = userVillage.buildings ? userVillage.buildings.filter(b => b.data === 1000010) : [];

    // Track wall distribution by level
    const wallsByLevel = {};
    ...
    result.details = {
        byLevel: wallsByLevel,
        totalWalls: totalWallCount,
        maxWalls: totalWallsAllowed,
