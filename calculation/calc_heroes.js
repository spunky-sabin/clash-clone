window.calculateHeroesCostBased = function (userVillage, staticData, userTH) {
    const result = {
        completed: 0,
        total: 0,
        percent: 0,
    ...
    if (staticData.heroes) {
        staticData.heroes.forEach(item => {
            ...
            if (userVillage.heroes) {
                const uHero = userVillage.heroes.find(h => h.data === item._id);
                if (uHero) {
                    const currentLvl = uHero.lvl || 1;
